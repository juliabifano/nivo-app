export default function CardSummary({
  ...
  if (temFatura) {
    ...
    // para débito/vale usa transações normais
    ...
    <div className="flex flex-col gap-5 mt-4 text-white">
      {temFatura && (
        <div className="bg-white/10 rounded-xl p-4 backdrop-blur">
          ...
          <p className="text-2xl font-semibold mt-1">{formatCurrency(usado)}</p>
      ...
      <div>
        <p className="text-sm text-white/70 mb-2">Últimas compras</p>
      ...
      {temFatura && (
        <div className="bg-white/5 border border-white/10 rounded-lg p-3">
          {percent > 80 && (
            <p className="text-red-400 text-sm">Alto uso do limite neste mês</p>
          )}
          ...
          {percent <= 50 && (
            <p className="text-emerald-400 text-sm">Uso controlado do cartão</p>
          )}
        </div>
      )}
    </div>
